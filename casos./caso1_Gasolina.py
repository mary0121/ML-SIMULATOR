...
buildingSurr = ['']*len(x)
sourceStrength = ['']*len(x)
store = ['']*len(x)

for i in range(len(x)):
    chemical[i] = 'Chlorine'
    sourceStrength[i] = 'cubic meters'
    store[i] = 'Liquid'
    ...
    if source[i] == 'Instantaneous':
        x['Duration (For)'][i] = 0
        amountPollutant[i] = random.randint(10, 300)
    else:
        amountPollutant[i] = random.randint(10, 300)/x['Duration (For)'][i]
...
x['Stored Themperature'] = x['Air themperature']
x['Source strength'] = sourceStrength
x['Stored'] = store

print(x)


read_write.write_csv(x,'AlohaDB3')
